...
# NEW: added new fields on top of original columns
CSV_COLUMNS = [
    "post_id", "run_id", "scraped_at",
    "price", "year", "make", "model", "mileage",
    "fuel_type", "transmission", "color",
    "num_doors", "cylinders", "drive_type", "condition",
    "source_txt"
]
...
def materialize_http(request: Request):
    ...
    and writes one CSV directly to .../datasets/listings_master_v3.csv.
    ...
    try:
        ...
        # NEW: writes to listings_master_v3.csv
        base = f"{STRUCTURED_PREFIX}/datasets"
        final_key = f"{base}/listings_master_v3.csv"
